...
class Goodbye(Window):
    def __init__(self, window_number, turns, screen):
        super().__init__()
        self.window_number = window_number
        self.turns = turns
        self.morph = pymorphy2.MorphAnalyzer()
        self.sq = self.morph.parse('ход')[0]
        self.running = True
        self.ui()
        self.run(screen)
    ...
    def run(self, screen):
        pygame.init()
        text = ["Поздравляю",
                "Вы прошли уровень за " + str(self.turns) + ' ' + self.sq.make_agree_with_number(self.turns).word]
